...
async fn make_pull_request_body(full_repo_name: &str, number: u64) -> Result<PullRequestEvent> {
    ...
    let gh = ofborg::hubcaps::Github::new("ofborg-send-event", None)?;
    let repo = gh.repo(org_name, repo_name);
    let pr = repo.pulls().get(number).get().await?;
    ...
    Ok(PullRequestEvent {
        ...
        pull_request: PullRequest {
            ...
            base: PullRequestRef {
                git_ref: pr.base.commit_ref,
                sha: pr.base.sha,
            },
            head: PullRequestRef {
                git_ref: pr.head.commit_ref,
                sha: pr.head.sha,
            },
        },
        changes: None,
...
async fn main() -> Result<()> {
    ...
    if let Some(secret_path) = &args.secret_path {
        let signature = github_signature_256(&std::fs::read_to_string(secret_path)?, &body)?;
        headers.insert(
